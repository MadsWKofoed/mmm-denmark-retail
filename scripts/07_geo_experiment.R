...
p_event <- ggplot(event_coefs, aes(week_rel, estimate)) +
  ...
  labs(title = "Event study: treated vs. control per-capita revenue, relative to week -1",
...
perm_estimates <- map_dbl(seq_len(n_perm), function(i) {
  ...
  if (is.null(fit) || !("treated_post_permTRUE" %in% names(coef(fit)))) return(NA_real_)
...
p_ri <- ggplot(tibble(estimate = perm_estimates), aes(estimate)) +
  ...
  labs(title = "Randomisation inference: null distribution of the DiD estimate",
...
boot_estimates <- map_dbl(seq_len(n_boot), function(i) {
  ...
  boot_panel <- map_dfr(seq_along(boot_munis), function(j) {
    panel |> filter(municipality == boot_munis[j]) |> mutate(municipality = paste0(municipality, "_", j))
  })
  fit <- tryCatch(feols(revenue_pc ~ treated_post | municipality + week_index, data = boot_panel, notes = FALSE),
                   error = function(e) NULL)
  if (is.null(fit) || !("treated_postTRUE" %in% names(coef(fit)))) return(NA_real_)
...
model_df <- as_tibble(media_mat) |> bind_cols(as_tibble(control_mat)) |>
...
saveRDS(list(fit = fit_calibrated, channel_summary = calibrated_summary, prior_mean = beta_mean, prior_sd = beta_sd),
...
p_calib_plot <- ggplot(p_calibration, aes(stage, estimate)) +
  ...
  labs(title = "social_prospecting: does the geo experiment improve ROAS recovery?",
